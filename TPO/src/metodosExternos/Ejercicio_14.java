...
/*public class Ejercicio_14 {
    public ConjuntoTDA VerticesPuente(GrafoTDA grafo, int p, int q) {
        ...
        while (!vertices.ConjuntoVacio()) {
            ...
            if (v != p && v != q) {
                GrafoTDA copia = CopiarGrafoSinVertice(grafo, v);
                if (!HayCamino(copia, p, q)) {
                    resultado.Agregar(v);
                }
            }
    ...
    private GrafoTDA CopiarGrafoSinVertice(GrafoTDA original, int excluido) {
        GrafoTDA copia = new Grafo();
        copia.InicializarGrafo();

        ConjuntoTDA vertices = original.Vertices();
        while (!vertices.ConjuntoVacio()) {
            int v = vertices.Elegir();
            vertices.Sacar(v);
            if (v != excluido) copia.AgregarVertice(v);
        ...
        while (!vertices.ConjuntoVacio()) {
            ...
            vertices.Sacar(v);
            if (v != excluido) {
                ConjuntoTDA ady = original.Adyacentes(v);
                while (!ady.ConjuntoVacio()) {
                    int w = ady.Elegir();
                    ady.Sacar(w);
                    if (w != excluido && copia.Pertenece(w)) {
                        copia.AgregarArista(v, w, 1);
                    }
                }
            }
        }
    ...
    private boolean HayCamino(GrafoTDA grafo, int p, int q) {
        ...
        while (!cola.ColaVacia()) {
            ...
            ConjuntoTDA ady = grafo.Adyacentes(actual);
            while (!ady.ConjuntoVacio()) {
                int vecino = ady.Elegir();
                ady.Sacar(vecino);
                if (!visitados.Pertenece(vecino)) {
